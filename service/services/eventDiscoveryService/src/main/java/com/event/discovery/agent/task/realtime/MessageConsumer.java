...
public class MessageConsumer extends RealtimeMessage implements TaskRunnerJob {
    public MessageConsumer(final EventDiscoveryContainer eventDiscoveryContainer, final LinkedBlockingQueue<NormalizedEvent> normalizedMessageProcessingQueue) {
    ...
    protected void poll() {
        while (!exit) {
            Object msg = getBrokerPlugin().getReceivedMessageQueue().poll();
            if (msg != null) {
                try {
